...
class LJSpeech(data.Dataset):
    ...
    def __init__(self, root, downsample=True, transform=None, target_transform=None, download=False, dev_mode=False):
        ...
        self.data, self.labels = torch.load(os.path.join(
            self.root, self.processed_folder, "ljspeech_{:04d}.pt".format(self.cached_pt)))
    ...
    def _check_exists(self):
        return os.path.exists(os.path.join(self.root, self.processed_folder, "ljspeech_info.txt"))

    def _write_info(self, num_items):
        info_path = os.path.join(
            self.root, self.processed_folder, "ljspeech_info.txt")
    ...
    def download(self):
        """Download the LJSpeech data if it doesn't exist in processed_folder already."""
        ...
        shutil.copyfile(
            ...
            os.path.join(processed_abs_dir, "LJSPEECH_COPYING")
        )
